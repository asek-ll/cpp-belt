...
private:
	...
	void Set(const std::size_t pos, const T &value) {
		if (coeffs_.size() <= pos && value != 0) {
			coeffs_.resize(pos + 1);
		}
		coeffs_[pos] = value;
		if (value == 0) {
			Shrink();
	...
	private:
		...
		std::size_t degree_;
	public:
		IndexProxy(std::size_t degree, Polynomial &poly): poly_(poly), degree_(degree) {
		}
		operator T() const {
...
public:
	...
	IndexProxy operator [](const size_t degree) {
		return IndexProxy(degree, *this);
	}
...
void TestCreation() {
	...
	}
	{
		...
		ASSERT(
				std::equal(cbegin(from_iterators), cend(from_iterators),
						begin(coeffs)));
	}
}
